...
![](../../../images/스크린샷%202026-05-31%2013.18.34.png)![](../../../images/스크린샷%202026-04-30%2022.11.22.png)
...
## ECS Rolling Update 한계

### ENI 개수 할당 문제
현재 t3.medium일 때 ENI 개수는 최대 3개이다. 하지만 롤링 업데이트 무중단 방식으로 app 컨테이너가 순간적으로 잠깐 ENI를 2개 사용하게 된다.컨테이너가 두 개가 동시에 뜨는 순간이 발생하기 때문에 ENI 개수를 2개 차지하게 되어 Redis 네트워크 모드를 bridge모드로 할 수 밖에 없는 상황이다.

### 최대 메모리 사용 문제
마찬가지로 컨테이너가 두 개가 동시에 뜨는 순간이 발생할 것이고, 인스턴스는 최대 메모리양이 정해져 있다. 각 ECS 태스크가 차지할 수 있는 최대 크기의 메모리와 CPU 용량도 정해져 있는데, 2개가 동시에 뜨는 상황이 있게 될 것이라서 인스턴스 메모리를 끝까지 채워서 사용할 수가 없다. 