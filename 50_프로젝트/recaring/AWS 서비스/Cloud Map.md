...
## Service Discovery 원리

ECS 태스크가 시작되거나 종료될 때 ECS가 CloudMap에 해당 태스크의 private IP를 자동으로 등록/해제 해준다. 그러면 CloudMap이 DNS A 레코드를 업데이트하고, Nginx가 resolver를 통해 그 DNS를 쿼리해서 최신 IP를 가져간다.
...
`$backend` : 변수를 사용하게 되면 요청이 들어올 때마다 변수가 가리키는 주소가 맞는 IP주소인지 다시 평가함
- 만약 도메인을 변수없이 하드코딩해서 넣으면 `valid` 를 무시하게 되어 맨 처음 IP로 영구히 트래픽을 보냄

`valid = 5s` : DNS 서버에서 IP를 받아오면, 결과값을 Nginx 메모리에 5초동안만 캐싱해 두는 의미
- **매 요청마다 주소를 다시 평가하면 부하 문제는 없나?**
	- 5초 동안의 모든 트래픽은 해당 IP로 요청을 보내게 됨
	- 5초가 지나기 전까지는 DNS 쿼리를 하지 않는다.
	- AWS 자체 네트워크에서 쿼리를 하기 때문에 1초마다 DNS 쿼리해도 괜찮음
