...
**인덱스 블록은 디스크에 있는 테이블 데이터 블록과 분리되어 있음. 인덱스 블록은 메모리에 있음!**
#### 인덱스로 테이블 블록을 액세스할 때
...
## 1. B-Tree vs B+Tree (인덱스의 자료구조)

관계형 DB는 왜 일반적인 이진 트리(Binary Tree)나 해시(Hash)가 아니라 B-Tree 계열, 그중에서도 **B+Tree**를 사용할까요? 핵심은 '디스크 I/O 최소화'와 '범위 검색(Range Scan)'에 있습니다.

### B-Tree (Balanced Tree)

- **특징:** 트리의 **모든 노드(Root, Branch, Leaf)에 실제 데이터(또는 포인터)가 저장**될 수 있습니다.
    
- **단점:** 중간 노드에 데이터가 공간을 차지하므로, 하나의 노드(페이지)에 들어갈 수 있는 '키(Key)'의 개수가 적어집니다. 트리의 깊이가 깊어져 디스크 탐색 횟수가 늘어날 수 있고, 범위 검색 시 트리를 오르락내리락해야 해서 비효율적입니다.
    

### B+Tree (DB 인덱스의 표준)

- **특징:** **오직 맨 끝의 리프 노드(Leaf Node)에만 실제 데이터(또는 포인터)를 저장**합니다. 중간 노드들은 오직 데이터가 어디 있는지 방향만 알려주는 '이정표(네비게이션)' 역할만 합니다.
    
- **장점:**
    
    1. 중간 노드에 이정표만 꽉꽉 채울 수 있어 트리의 깊이(Depth)가 얕아집니다 (디스크 I/O 감소).
        
    2. 리프 노드끼리 Linked List(연결 리스트)로 이어져 있습니다. 범위 검색을 할 때 리프 노드에 도달한 후 옆으로 쭉 훑기만 하면 되므로 극도로 빠릅니다.
        

> 🗣️ **면접 답변 예시 & 비유** "B-Tree와 B+Tree의 가장 큰 차이는 **실제 데이터의 저장 위치**와 **리프 노드의 연결 여부**입니다. 예를 들어, **'연봉 5천만 원부터 8천만 원 사이의 직원'**을 찾는다고 가정해 보겠습니다. B-Tree는 5천, 6천, 7천만 원을 찾을 때마다 매번 트리의 위아래를 반복해서 탐색해야 합니다. 반면, B+Tree는 한 번 트리를 타고 내려가 5천만 원을 찾은 뒤에는, 리프 노드에 연결된 리스트를 따라 8천만 원이 나올 때까지 **수평으로 쭉 스캔(가로 탐색)**만 하면 되므로 DB의 범위 검색에 훨씬 유리합니다."

---

## 2. 클러스터(Clustered) 인덱스 vs 논클러스터(Non-Clustered) 인덱스

이 둘은 인덱스가 '실제 데이터를 어떻게 물리적으로 저장하고 가리키는가'에 대한 차이입니다.

### 클러스터 인덱스 (Clustered Index)

- **특징:** **테이블의 실제 데이터가 인덱스의 정렬 순서대로 물리적으로 정렬**되어 저장됩니다.
    
- **리프 노드의 정체:** 클러스터 인덱스의 리프 노드는 **곧 실제 데이터(Row) 전체**입니다.
    
- **제약:** 물리적인 순서를 결정하므로 **테이블당 딱 1개**만 만들 수 있습니다. (주로 Primary Key가 클러스터 인덱스가 됩니다).
    

### 논클러스터 인덱스 (Non-Clustered Index / Secondary Index)

- **특징:** 실제 데이터의 물리적 순서와는 무관하게, **인덱스만을 위한 별도의 공간**을 만들어 정렬합니다.
    
- **리프 노드의 정체:** 리프 노드에는 실제 데이터가 없습니다. 대신 실제 데이터가 어디 있는지 찾아갈 수 있는 '포인터(또는 PK 값)'만 들어있습니다.
    
- **제약:** 테이블당 여러 개를 만들 수 있습니다.
    

> 🗣️ **면접 답변 예시 & 비유** "클러스터 인덱스는 **'국어사전'**과 같습니다. '가나다' 순으로 인덱스(단어)가 정렬되어 있고, 그 단어 옆에 바로 실제 데이터(뜻풀이)가 물리적으로 함께 적혀 있습니다. 테이블당 1개만 존재하며, 찾고자 하는 위치에 도달하면 즉시 데이터를 얻을 수 있어 매우 빠릅니다.
> 
> 반면, 논클러스터 인덱스는 **'전공 서적 맨 뒤의 찾아보기(색인)'**와 같습니다. 키워드는 '알파벳' 순으로 정렬되어 있지만, 옆에는 실제 내용 대신 **'페이지 번호(포인터)'**만 적혀 있습니다. 키워드를 찾은 후 해당 페이지 번호를 가지고 본문을 다시 한 번 펼쳐야(추가 디스크 I/O) 실제 데이터를 읽을 수 있습니다. 대신 여러 기준(저자별, 주제별 찾아보기 등)으로 여러 개를 만들 수 있습니다."