...
- ALL 
	- Table Full Scan
	- 인덱스가 있더라도, 전체 데이터의 20프로 이상을 읽어야 하게 되면 테이블 full scan이 적용됨
- range
	- 인덱스를 타면서 딱 하나의 값이 아닌, 범위 구간을 탐색할 때 사용
- ref(Non-Unique Index Scan : 중복된 값이 들어갈 수 있는 인덱스) 
	- 중복된 값이 여러개인 상황에서 사용
	- 동등조건을 만족하는 여러 개의 데이터 묶음일때 사용
...
## 운영환경에서 인덱스 추가할 때 고려해야 할 사항

### 1. 테이블 락
- **문제** : DB는 인덱스를 만들기 위해 기존 데이터를 전부 읽고 정렬해야 한다. 이때 데이터 일관성을 유지하기 위해 테이블에 쓰기 작업을 막는 락을 걸어버린다. 이렇게 되면 인덱스 생성을 하는 시간동안 모든 API 요청이 실패할 것이다.
- **해결책** : 서비스 중단 없이 Online DDL 기능으로 `ALGORITHM=INPLACE, LOCK=NONE` 옵션을 붙여서 락이 걸리지 않도록 한다.
