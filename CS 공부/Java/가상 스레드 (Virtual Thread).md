...
1. **`synchronized` 블록 또는 메서드 내부에서 블로킹 I/O작업을 수행할 때**
	...
	- JVM은 현재 실행 중인 OS 스레드 자체에 락을 묶어버린다.
	- JVM이 해당 OS 스레드에 락을 걸어버리면 엮여있는 캐리어 스레드, 가상 스레드 전부 하나의 락으로 보게 되어서 가상 스레드가 해당 캐리어 스레드에 고정되어버린다.
...
- `ReentrantLock` : 락 소유자를 OS 스레드가 아닌 가상 스레드 객체 기준으로 추적
	- 락 소유자가 가상 스레드 객체 자체이기 떄문에, 캐리어 스레드를 바꿔도 락이 깨지지 않게 되어 자유롭게 I/O 대기 중에 unmount 할 수 있다.
