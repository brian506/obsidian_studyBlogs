...
서브넷 : 하나의 서브넷은 하나의 가용영역(AZ)안에 위치

**서브넷 가용 영역 크기 구하는 방법**
$$2^{(32 - N)}$$
- 여기서 N은 슬래시 뒤의 숫자
ex) 10.100.100.0/24 -> 총 256(2^8)
- 사용할 수 있는 IP는 256 - 5 = 251 개!
...
- VPC에 있는 가상의 라우터로 서브넷에서 오고가는 트래픽을 라우팅
	- 모든 서브넷의 트래픽은 VPC 라우터를 거쳐서 목적지에 도달

**중요**
- 같은 VPC 내 서버들은 각각 퍼블릭/프라이빗 네트워크 존재 가능
	- 프라이빗IP : VPC 내부에서 서버끼리 통신할 때 사용
		- 같은 VPC 내 서버들은 서로 프라이빗 IP로 통신
	- 퍼블릭IP : 외부 인터넷과 통신할 때 사용
- VPC 안에 있는 자원이 VPC 밖에 있는 AWS 자원과 통신할 떄(서로 다른 VPC끼리 통신할 때) **VPC 엔드포인트** 방식으로 통신
	- 이건 비용 발생함
...
>EC2 인스턴스 바로 앞에서 트래픽 필터링

**용도** : 특정 애플리케이션 서비스 (Spring 8080 포트 등)에 접근할 수 있는 대상을 정할 때 
**특징** : 인바운드를 정하면, 아웃바운드는 자동으로 허용

#### 네트워크 ACL (NACL)
> 서브넷 전체를 감싸서 필터링

**용도** : 특정 IP 대역 전체를 차단하거나, 서브넷 단위의 보안 정책을 세울 때 사용
**특징** : 인바운드와 아웃바운드 독립적
