...
---
일반적인 로드 밸런서(ALB, NLB)는 **전달자**가 아니라 **대리인(Proxy)** 역할을 합니다.

- **상황:** 클라이언트가 웹 서버에 접속하려고 합니다.
    
- **작동 방식:** 1. 클라이언트가 로드 밸런서(LB)에게 "연결하자!"라고 요청합니다. (**TCP 3-Way Handshake 종료**) 2. LB가 그 요청을 수락하고 연결을 맺습니다. (여기서 원래의 패킷 흐름은 **종료**됩니다.) 3. LB가 내부 상황을 보고, 웹 서버에게 **"자, 이제 내가 새로 연결을 요청할게"**라며 새로운 패킷을 보냅니다.
    

즉, 패킷의 **'연속성'**이 끊기고 두 개의 별도 연결(클라이언트-LB / LB-서버)이 생기는 것이죠.

패킷의 '보내는 사람(Source IP)' 주소가 바뀌어버려요.

- **원본 패킷:** [보내는 사람: 해커(1.2.3.4)] $\rightarrow$ [받는 사람: 로드 밸런서(5.5.5.5)]
    
- **LB가 새로 만든 패킷:** [보내는 사람: **로드 밸런서(5.5.5.5)**] $\rightarrow$ [받는 사람: 웹 서버(10.0.1.5)]
---
### 5. GWLB 상세
