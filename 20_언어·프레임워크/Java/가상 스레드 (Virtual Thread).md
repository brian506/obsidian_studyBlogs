...
위 그림을 기준으로 참고해서 동작원리에 대해 알아보자. (여기서 *자바 스레드와 플랫폼 스레드는 같은 것*)
OS 커널에서 할당 받은 OS 스레드와 JVM 스레드는 1대1 매핑관계이다. 

만약 외부 API를 호출한다고 가정해보자.
 1. JVM 스레드는 먼저 OS 스레드에게 작업 공간을 요청한다. (System Call)
 2. OS는 커널을 움직여 OS 메모리에 1MB짜리 스택을 할당하고 OS 스레드를 **새로** 생성한다.
 3. OS 스레드 A는 미리 만들어 둔 스레드 풀에서JVM 스레드 A를 할당 받아 일을 시작한다.
 4. JVM 스레드인 Platform 스레드는 외부 API 응답을 받을 때까지 기다려야 한다.(Blocked I/O)
 5. 그와 동시에 OS 스레드 A도 할 일이 없어져 버리기 때문에 OS 스레드A는 Blocking 상태가 되어 휴식에 취한다.
 6. 이때 CPU가 가만히 있지 않게 하기 위해서 컨텍스트 스위칭(context switching)이 발생하여 다른 OS 스레드 B를 일하도록 한다.
	 - OS 스레드는 수십~수천 개 존재할 수 있지만, **동시에 실행(run state) 가능한 스레드 수는 CPU 코어 수**와 같다.
7. 응답이 도착하면, 다시 OS 스레드 A를 깨우고 남은 작업을 이어간다.
8. 모든 작업이 완료되면, 스레드 풀이 반납하고 대기시킨다.
...
2. `Fork Join Pool`은 비어있는 `Platform Thread A` 를 찾아 가상 스레드A를 Mount 해준다. (CPU 코어 개수만큼의 플랫폼 스레드를 미리 생성 -> OS 스레드와 1대1로 영구 매핑되어 있음)
	- 여기서 연결된 플랫폼 스레드 = 캐리어 스레드
3. 가상 스레드A가 외부 API 호출을 하면 가상 스레드는 응답이 올때까지 기다린다.(I/O Block)
