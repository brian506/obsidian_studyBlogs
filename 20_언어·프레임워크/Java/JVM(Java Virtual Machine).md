...
2. **Java Compiler** 가 소스코드 .java 파일을 JVM만 읽을 수 있는 .class 파일인 Byte Code 로 컴파일한다.
...
: JVM 내로 클래스 파일(.class) 을 **동적으로 로드**하고, 링크를 통해 배치하는 작업을 수행하는 모듈이다.
...
`static Boy boy = new Boy("brian",27)` 
- 힙 영역에는 Boy 객체, 메서드 영역에는 boy 변수
**메서드 영역에 있는 boy 변수가 힙에 있는 Boy 객체를 가리키기 때문에, 프로그램이 종료될 때까지 GC의 수거 대상이 되지 않는다**
...
큰 껍데기를 미리 만들어두고 해당 주소를 사용하는 느낌.
...
자바는 기본적으로 멀티스레드 환경이다. CPU 코어는 **한 번에 하나의 스레드만** 실행할 수 있기 때문에, 여러 스레드를 번갈아가며 실행하는 컨텍스트 스위칭이 일어난다.
- 스레드 A를 실행하다가 스레드 B로 넘어갔다가 다시 **스레드 A로 돌아왔을 때 현재 위치를 저장**해두는 공간이 PC Register이다. (스레드마다 1개씩 존재)

### [Native Method Stack]

: 자바 코드가 컴파일되어 생성되는 바이트 코드가 아닌 실제 실행할 수 있는 기계어(C/C++ 등)로 작성된 프로그램을 실행시키는 영역
- 하드웨어 영역(네트워크 소켓, 파일 읽기/쓰기, 스레드 생성 등)을 제어하기 위해 존재
...
- JVM 개념과 역할에 대해 설명하라
- JVM의 작동 방식에 대해 설명하라
- Heap 영역과 Stack 영역이 어떻게 참조되고 사용되는지 설명해라
