...
보통 dev 환경에서 로그 레벨 제한하고 prod 레벨에서는 코드 상태를 알기 위해 DEBUG까지 해둔다.

**TRACE, DEBUG** : 3일치 보관, 개발 중 코드의 상태나 흐름을 이해하기 위함
**INFO** : 정보성 로그 (중요한 이벤트 or 상태 변화 기록)
**WARN** 
- 1분간 10회 이상 발생시 알람,
- 잠재적으로 문제가 될 경우(시스템 운영에는 즉각적인 영향 안줄때)
**ERROR** 
-  1회라도 발생 시 알람
- 복구가 필요하거나 실패한 작업을 추적할 때
**FATAL** : 존나 심각
