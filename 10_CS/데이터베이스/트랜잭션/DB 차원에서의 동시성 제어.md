...
[[../../Java/영속성 컨텍스트(Persistance Context)]] - 참고
...
이를 해결하기 위해 락을 사용할 수 있지만, 락을 사용하게 되면 대기시간이 발생해 병목지점이 뚜렷하게 발생한다.
영속성 컨텍스트를 사용하지 않고 DB 로 바로 `UPDATE`쿼리를 날려서 동시성 제어를 할 수 있는 방법을 알아보자.
