...
[데이터베이스 저장 구조 및 조회 방식](../데이터베이스%20저장%20구조%20및%20조회%20방식.md) -참고
...
## REPEATABLE READ  와 READ COMMITTED 차이

`REPEATABLE READ` 는 트랜잭션이 시작한 시점의 undo 로그를 읽는다.
`READ COMMITTED` 는 커밋된 데이터의 undo 로그만 읽는다.

ex ) REPEATABLE READ 예시
1. 트랜잭션 1이 영민이의 계좌를 조회한다. (잔액 : 만원) - 해당 트랜잭션 아직 끝나지 않음
2. 트랜잭션 2가 영민이의 계좌에서 5천원을 출금하고`COMMIT`한다. (잔액 : 5천원)
3. 트랜잭션 1이 영민이의 계좌를 다시 조회한다. (여전히 잔액 만원)
첫 조회 시 잔액 만원 -> 두 번째 조회 시 잔액 ( **잔액 변동 없음 : 만원**)

ex) READ COMMITTED 예시
1. 트랜잭션 1이 영민이의 계좌를 조회한다. (잔액 : 만원) - 해당 트랜잭션 아직 끝나지 않음
2. 트랜잭션 2가 영민이의 계좌에서 5천원을 출금하고 `COMMIT` 한다. (잔액 : 5천원)
3. 트랜잭션이 1이 영민이의 계좌를 다시 조회한다. (잔액 : 5천원)
첫 조회 시 잔액 만원 -> 두 번째 조회 시 잔액 (**만원 -> 5천원**)
