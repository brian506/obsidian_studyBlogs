...
**발생 조건** : **PK 가 아닌 일반 인덱스**로 범위를 조회하거나, 존재하지 않는 값을 조회할 때
...
**발생 조건** : `REPEATABLE READ` 격리 수준에서 일반 인덱스를 사용해 조회/수정할 때 
...
- 다른 트랜잭션은 C공연 레코드를 수정할 수 없고, stock = 100~150 인 레코드에 대해서도 수정 불가
