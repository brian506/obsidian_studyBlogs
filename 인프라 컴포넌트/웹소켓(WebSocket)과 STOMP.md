...
![사진](../images/kafka2.png)
___
## 웹소켓 세션

 **세션 생성** : 웹소켓 핸드셰이크가 이루어지고 서버와 연결이 맺어지면 서버는`WebSocketSession(sessionId)` 을 생성
	- 이 세션은 TCP 소켓 연결과 1대1 매핑

**세션 저장** : 
- Spring이 내부적으로 관리하는 `sessionAttributes(Map<String,Object>` 에 저장
	- `sessionId`, `Principal`, 연결 상태 등이 자동으로 들어감
	- `roomId`같은 값은 직접 넣어야함
```java
StompHeaderAccessor accessor = StompHeaderAccessor.wrap(event.getMessage());
accessor.getSessionAttributes().put("roomId", roomId);
```

**세션 전송** : `SEND`프레임 보낼 때 항상 해당 세션을 통해 전송
	- `sessionId`값으로 사용자 식별
