...
`acks=0`은 Producer가 응답을 기다리지 않습니다. 가장 빠르지만 메시지 유실 가능성이 있습니다.

`acks=1`은 Leader 파티션에만 저장되면 응답합니다. 속도와 안정성의 중간입니다. Leader가 장애나면 유실 가능성이 있습니다.

`acks=all(-1)`은 Leader + 모든 ISR(In-Sync Replica)에 저장되어야 응답합니다. 가장 안전하지만 그만큼 latency가 높아집니다.
...
관리자는 창고 내의 상자들이 어떤 상태인지, 어디에 위치해 있는지,  어떤 소비자가 어떤 상자를 사용하는지 파악해야 한다. 이 관리자의 역할을 [[Zookeeper  vs KRaft]] 로 비유할 수 있다.
